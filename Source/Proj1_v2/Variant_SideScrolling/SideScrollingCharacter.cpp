...
ASideScrollingCharacter::ASideScrollingCharacter()
{
	...
	//// added code
	// debug print
	// GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, TEXT("Finished Constructor"));
}
...
void ASideScrollingCharacter::DoDashStart()
{
	...
	if (bCanDash == true)
	{
		...
		if (MovementVelocityVector.X != 0)
		{
			...
			// debug prints
			// GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, TEXT("Started dash with Vector: {x}=%f,{y}=,{z}="), DashLaunchVector.X);
			//UE_LOG(LogTemp, Warning, TEXT("Started dash with Vector: {x}=%f,{y}=,{z}="), DashLaunchVector.X);
		}
	}
}
